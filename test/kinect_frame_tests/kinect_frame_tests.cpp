...
{
    ASSERT_NO_THROW(
        KinectFrame kinect_frame(1920,1080);
    );
}

TEST_F(KinectFrameTest, Fill)
{
    KinectFrame kinect_frame(width, height);
...
TEST_F(KinectFrameTest, ComputeDifferences1)
{
    ...
    KinectFrame kinect_frame_1(width, height);
    KinectFrame kinect_frame_2(width, height);

    kinect_frame_1.Fill(test_data_1.data(),0);
    kinect_frame_2.Fill(test_data_2.data(),0);

    EXPECT_EQ(kinect_frame_1.ComputeDifferences(kinect_frame_2, tolerance), width * height);
    EXPECT_EQ(kinect_frame_2.ComputeDifferences(kinect_frame_1, tolerance), width * height);
}
...
TEST_F(KinectFrameTest, ComputeDifferences2)
{
    ...
    EXPECT_EQ(kinect_frame_1.ComputeDifferences(kinect_frame_2, tolerance), 0);
    EXPECT_EQ(kinect_frame_2.ComputeDifferences(kinect_frame_1, tolerance), 0);
}
...
TEST_F(KinectFrameTest, ComputeDifferences3)
{
    ...
    EXPECT_EQ(kinect_frame_1.ComputeDifferences(kinect_frame_2, tolerance), num_differences);
    EXPECT_EQ(kinect_frame_2.ComputeDifferences(kinect_frame_1, tolerance), num_differences);
}
